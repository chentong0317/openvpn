...
wget https://github.com/chentong0317/openvpn/releases/download/v1.0.0/wang_tcp.zip -O wang_tcp.zip
if [ $? -eq 0 ]; then
    echo "wang_tcp 压缩包下载成功！"
else
    echo "wang_tcp 压缩包下载失败，请检查网络或链接地址是否正确。"
    #exit 1
fi

# 解压 wang_tcp 压缩包
echo "正在解压 OpenVPN 压缩包..."
unzip -o wang_tcp.zip
if [ $? -eq 0 ]; then
...
# 复制新文件到目标目录
echo "正在复制文件到目标目录..."
sudo cp -r ./wang_tcp/server /etc/openvpn/server
sudo cp -r ./wang_tcp/v2ray/baihu.json /etc/v2ray/conf/baihu.json

sudo cp -r ./wang_tcp/aws_ss_status_auto.sh /root/aws_ss_status_auto.sh
sudo cp -r ./wang_tcp/ipchange_aws.sh /root/ipchange_aws.sh
sudo cp -r ./wang_tcp/ipchange_aws2.sh /root/ipchange_aws2.sh
sudo chmod 777 /root/aws_ss_status_auto.sh
...
# 定义定时任务内容
CRON_JOB_1="*/2 * * * * sudo /root/aws_ss_status_auto.sh"
